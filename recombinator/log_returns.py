...
from recombinator.utilities import BlockBootstrapType
...
def calculate_number_of_complete_blocks(sample_length: int, block_length: int) -> int:
    """
...
def _get_number_of_observations_from_1d_or_2d_array(
    a: tp.Union[ArrayInt, ArrayFloat]
) -> int:
    """
    ...
    if a.ndim == 1:
        ...
        raise ValueError("The data array must be a NumPy array with one or two axes")

    return T


def remove_incomplete_blocks(
    log_returns: tp.Union[ArrayInt, ArrayFloat],
    ...
    """
    T = _get_number_of_observations_from_1d_or_2d_array(log_returns)

    number_of_complete_blocks = calculate_number_of_complete_blocks(
        T, number_of_observations_to_aggregate
    )
    end_index = number_of_complete_blocks * number_of_observations_to_aggregate
    result_log_1_array: tp.Union[ArrayInt, ArrayFloat] = log_returns[:end_index]
    result_log_2_array: tp.Union[ArrayInt, ArrayFloat] = log_returns[:, :end_index]
    if log_returns.ndim == 1:
        return result_log_1_array
    elif log_returns.ndim == 2:
        return result_log_2_array
    else:
        assert False
...
def aggregate_log_returns(
    ...
    if number_of_observations_to_aggregate < 1:
        raise ValueError(
            "The number of observations to aggregate must be a positive integer."
        )

    if number_of_observations_to_aggregate == 1:
        return log_returns, log_returns

    n_observations = _get_number_of_observations_from_1d_or_2d_array(log_returns)

    if rolling:
        if log_returns.ndim == 1:
            log_returns_df = pd.Series(log_returns)
            aggregated_log_returns = (
                log_returns_df.rolling(number_of_observations_to_aggregate)
                .sum()
                .dropna()
                .values
            )
        elif log_returns.ndim == 2:
            rolling_log_returns = rolling_window(
                log_returns, number_of_observations_to_aggregate
            )
            aggregated_log_returns = rolling_log_returns.sum(axis=2)
        else:
            raise ValueError(
                "The argument log_returns must be a one- or two-dimensional array"
            )
        ...
        if log_returns.ndim == 1:
            ...
            retained_log_returns = log_returns[:end_index]
            aggregated_log_returns = retained_log_returns.reshape(
            ...
            aggregated_log_returns = retained_log_returns.reshape(
                -1, number_of_complete_blocks, number_of_observations_to_aggregate
            ).sum(axis=2)
        else:
            raise ValueError(
                "The argument log_returns must be a NumPy array with one or two axes"
            )

    if verbose:
        print(
            f"Number of cut off observations "
            f"= {n_observations % number_of_observations_to_aggregate}"
        )
...
def aggregate_multi_dimensional_log_returns(
    ...
    number_of_complete_blocks = calculate_number_of_complete_blocks(
        sample_length=T, block_length=number_of_observations_to_aggregate
    )

    if number_of_complete_blocks * number_of_observations_to_aggregate != T:
        raise ValueError("T / number_of_observations_to_aggregate must be an integer")

    aggregated_log_returns: tp.Union[ArrayInt, ArrayFloat] = (
        log_returns.reshape(
            (
                replications,
...
def _calculate_number_of_observations_to_be_cut_off_at_the_end(
    sample_length: int, number_of_observations_to_aggregate: int, verbose: bool = False
) -> int:
    """
    ...
    obs_remain_end = sample_length % number_of_observations_to_aggregate

    obs_to_be_cut_off_end = (obs_remain_end + 1) % number_of_observations_to_aggregate

    if verbose:
        print(f"observations remaining at the end " f"= {obs_remain_end}")
        print(obs_to_be_cut_off_end)

    return obs_to_be_cut_off_end
...
def resample_and_aggregate(
    ...
    replications: int,
    number_of_observations_to_aggregate: int,
    block_bootstrap_type: BlockBootstrapType = BlockBootstrapType.STATIONARY,
    ...
    aggregated_log_returns, retained_log_returns = aggregate_log_returns(
        log_returns=log_returns,
        number_of_observations_to_aggregate=number_of_observations_to_aggregate,
        rolling=rolling,
        verbose=verbose,
    )
    ...
    if block_length == 1:
        resampled_log_returns = iid_bootstrap(
        ...
        resampled_log_returns = moving_block_bootstrap(
            retained_log_returns,
        ...
        resampled_log_returns = circular_block_bootstrap(
            retained_log_returns,
            block_length=block_length,
            replications=replications,
            sub_sample_length=sub_sample_length,
        ...
        resampled_log_returns = stationary_bootstrap(
            retained_log_returns,
            block_length=block_length,
            replications=replications,
            sub_sample_length=sub_sample_length,
        ...
        resampled_log_returns = tapered_block_bootstrap(
            retained_log_returns,
            block_length=block_length,
            replications=replications,
            sub_sample_length=sub_sample_length,
    ...
    aggregated_resampled_log_returns, _ = aggregate_log_returns(
...
def resample_and_aggregate_multidimensional_log_returns(
    ...
    num_complete_blocks = calculate_number_of_complete_blocks(
        sample_length=T, block_length=number_of_observations_to_aggregate
    )
    num_of_obs_to_retain = num_complete_blocks * number_of_observations_to_aggregate

    if block_length == 1:
        resampled_log_returns = iid_bootstrap(
            log_returns,
            replications=replications,
            sub_sample_length=num_of_obs_to_retain,
        )
        ...
        resampled_log_returns = tapered_block_bootstrap_vectorized(
            log_returns,
            block_length=block_length,
            replications=replications,
            sub_sample_length=num_of_obs_to_retain,
    ...
    aggregated_resampled_log_returns = aggregate_multi_dimensional_log_returns(
        log_returns=resampled_log_returns,
        number_of_observations_to_aggregate=number_of_observations_to_aggregate,
    )
